...
public class PasswordEvaluator {
	...
	public static boolean foundOtherChar = false;       // *** CHANGE: Added otherChar variable ***
	...
	public static String evaluatePassword(String input) {
		...
		foundOtherChar = false;             // *** CHANGE: Reset otherChar flag ***
		running = true;						// Start the loop

		// The Directed Graph simulation continues until the end of the input is reached or at some 
		// state the current character does not match any valid transition
		while (running) {
			displayInputState(); // Call debugging information
			...
			if (currentChar >= 'A' && currentChar <= 'Z') {
				...
				System.out.println("Invalid character found"); // *** CHANGE: Log invalid character ***
				foundOtherChar = true;                         // *** CHANGE: Set otherChar flag ***
				passwordIndexofError = currentCharNdx;
				return "*** Error *** An invalid character has been found!";
			}

			// Check if the password has reached 8 characters
			if (currentCharNdx >= 7) {
		...
		// Ensure error messages indicate missing conditions
		if (!foundUpperCase) errMessage += " Missing upper case \n";
		if (!foundLowerCase) errMessage += "Missing lower case \n";
		if (!foundNumericDigit) errMessage += "Missing numeric digits \n";
		if (!foundSpecialChar) errMessage += "Missing special character \n";
		if (!foundLongEnough) errMessage += "Password is too short \n";
		if (foundOtherChar) errMessage += "Invalid characters detected \n"; // *** CHANGE: Add otherChar check ***
